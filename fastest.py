...
def search_for_counterexample(base_graph, name):
    ...
    # HFT-Opt: Pre-compute all subgraph edgesets and group them by size.
    # This avoids re-calculating or filtering the full 2^E list repeatedly.
    print("Pre-computing and indexing all subgraph edgesets by size...")
    subgraphs_by_size = defaultdict(list)
    ...
    for i in range(1 << eG):
        ...
        subgraphs_by_size[len(edgeset)].append(edgeset)
        all_subgraph_edgesets.append(edgeset)
    all_subgraphs_set = set(all_subgraph_edgesets)
    print(f"Generated and indexed {len(all_subgraph_edgesets)} subgraph edgesets.")
    ...
    for i, p3_seed_edges in enumerate(all_p3_seeds):
        # HFT-Opt: Directly construct the trivial family, don't filter 2^E graphs.
        other_edges = [e for e in base_edges if e not in p3_seed_edges]
        F_trivial = []
        for j in range(1 << len(other_edges)):
            superset_edges = p3_seed_edges.union(other_edges[k] for k in range(len(other_edges)) if (j >> k) & 1)
            F_trivial.append(superset_edges)
        
        # HFT-Opt: Use fast set difference to find candidates.
        trivial_set = set(F_trivial)
        candidates = sorted(list(all_subgraphs_set - trivial_set), key=len, reverse=True)
    ...
    for k in range(eG, 2, -1):
        ...
        # HFT-Opt: Use the pre-computed index to build the kernel instantly.
        F_kernel = [es for size in range(k, eG + 1) for es in subgraphs_by_size[size]]
        ...
        # HFT-Opt: Sort once, then filter.
        candidates = [es for es in all_subgraph_edgesets if es not in augmented_keys]
        candidates.sort(key=len, reverse=True)
        
        for new_es in candidates:
            if check_new_edgeset_against_family(new_es, F_augmented, memo):
                F_augmented.append(new_es)
                augmented_keys.add(new_es) # Keep the lookup set in sync
